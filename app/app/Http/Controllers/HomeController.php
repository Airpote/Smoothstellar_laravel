...
class HomeController extends Controller
{
    ...
    public function get_stellarBalance(){
        $id=Auth::id();
        $client_public_key = User::whereId($id)->first()->stellarwallet;
        $server = Server::publicNet();
        $account = $server->getAccount($client_public_key);
        foreach ($account->getBalances() as $balance) {
            if($balance->getAssetCode()==$currency->symbol){
                $smc_balance=$balance->getBalance();
                break;    
            }    
        }
    ...
    public function getBalance()
    {
        ...
        foreach($stellarcoin as $stellar){
            $baseUrl="https://api.stellar.expert/explorer/public/asset/";
            $endpoint=$stellar->AssetCode."-".$stellar->AssetID;
            $ch = curl_init();
            ...
            Stellar::find($stellar->id)->update(array(
                'price'=>$content['price'],
            ));
        }
        $coin=['BTC','LTC','DOG'];
        ...
        for($i=0;$i<2;$i++){
            $endpoint = "/rates/merchant/".$coin[$i]."/USD";
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $baseUrl.$endpoint);
            curl_setopt($ch, CURLOPT_HTTPGET, true);
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
            ...
            curl_close($ch);
            Coin::whereCurrency($coin[$i])->update(array(
                'price'=>$content
            ));
        }
    }
...
//
  ...
  public function logtoggle(Request $request)
    ...
    {   
        $id=Auth::user()->id;
        $address=user::whereId($id)->first();
        $data['address']=$address->stellarwallet;
        $data['paypal']=$address->paypal;
    ...
    public function postbank(Request $request)
    {
        ...
        if(isset($request->wallet)){
            ...
            if(isset($request->btc)){
                $user->stellarwallet = $request->btc;
                $user->save();
            }
...
$charge = $gate->fixed_charge + ($request->amount * $gate->percent_charge / 100);
 ...
 public function vieworder(Request $request)
  ...
  public function sellmarketpost(Request $request)
    ...
    //         if($request->gateway ==  101){
    //             $gate = Gateway::whereId(101)->first();
    //         }
  ...
  public function replyticket(Request $request)
    ...
    //     $baseUrl = "https://api.coingate.com/v2";
    //     $endpoint = "/rates/merchant/ETH/USD";
    //     $httpVerb = "GET";
    //     $contentType = "application/json"; //e.g charset=utf-8
    //     $headers = array (
    //         "Content-Type: $contentType",
...
//My function
    ...
    //     $get['currency'] = Currency::whereStatus(1)->orderBy('id')->first();
    //     $get['method'] = PaymentMethod::whereStatus(1)->orderBy('name','asc')->get();
    //     $get['bank'] = Bank::whereStatus(1)->orderBy('name','asc')->get();
    //     $get['page_title'] = " Buy Smooth Coin";
    //     return view('user.sbuy', $get);
    // }

//////////////////////// Buy Smoothcoin////////////////////////
    public function buybscoin()
    {    
        $auth = Auth::user();
        $id=Auth::id();
        if ($auth->verified != 2 ){
        ...
        $get['address']=$user->stellarwallet;
        $get['paypal_address']=$user->paypal;
        return view('user.bsbuy', $get);
    }


    public function buybsecoin(Request $request)
    {
        $auth = Auth::user();
        if ($auth->verified != 2 ){
            return back()->withAlert('You are not eligible to buy cryptocurrency. Please verify your account first');
        ...
        if($request->flag==4){
            ...
            $buy['user_id'] = Auth::id();
            $buy['type'] = 1;
            $buy['wallet'] = $request->paypal_address;
            $buy['gateway'] = 101;
            $buy['remark'] = "$";
            $buy['status'] = 0;
            $buy['trx'] = $trx;
            $data = Trx::create($buy)->trx;
            Session::put('Track', $buy['trx']);
            return redirect()->route('user.ebuypaypal', $trx);
        }
        elseif($request->flag==2){
            $coindata=Stellar::whereId($request->gateways)->first();
            //get the stellar balace
            $client_public_key = $request->wallet;
            $client_private_key=$request->account_info;
            $server = Server::publicNet();
            //Account Exist
            // try {
            //     $exists = $server->accountExists($request->wallet);
            
            //     if ($exists) {
            //     }
            //     else {
            //         return back()->with("alert", "Account does not exist!");
            //     }
            // }
            // catch (\Exception $e) {
            ...
            $flag==0;
            foreach ($account->getBalances() as $balance) {
                if($balance->getAssetCode()==$coindata->AssetCode){
                    if($balance->getBalance()>=$request->pay_amount){
                        $flag=1;
                        break;
                    }
                }    
            ...
            If(flag==0){
                return back()->with("alert", "You dont have enough ".$coindata->AssetCode ." in your stellar wallet.Please deposit more ".$coindata->AssetCode." or try using another payment gateway"); 
            }
            //end balance

            //payment
            $usd = $request->pay_amount;
            $send_coin = $usd + $charge/$coindata->price;
            $asset = Asset::newCustomAsset($coindata->AssetCode, $coindata->AssetID);
                try {
                    $response = $server->buildTransaction($client_public_key)
                    ->addCustomAssetPaymentOp($asset, $send_coin, $admin_public_key) // this will default to the maximum value
                    ->getTransactionEnvelope();
                    $response->sign($client_private_key);
            ...
            $get=$request->amount;
            $buy['currency_id'] = $currency->id;
            $buy['amount']=$usd*$coindata->price;
            $buy['main_amo'] = $send_coin*$coindata->price;
            $buy['charge'] = $charge;
            $buy['price'] = $currency->price;
            $buy['getamo'] = $get;
            $buy['user_id'] = Auth::id();
            $buy['type'] = 1;
            $buy['wallet'] = $request->wallet;
            $buy['gateway']=$coindata->gateway_id;
            $buy['remark'] = $coindata->AssetCode."(stellar)";
            $buy['status'] = 1;
            $buy['amountpaid'] = $send_coin;
            $buy['trx'] = $trx;
            $data = Trx::create($buy)->trx;
            Session::put('Track', $buy['trx']);
            ...
            $asset = Asset::newCustomAsset('SMC', $issuingAccountID);
                ...
                try {
                    ...
                    ->addCustomAssetPaymentOp($asset, $get, $client_public_key) // this will default to the maximum value
                    ->getTransactionEnvelope();
        ...
        elseif($request->flag==3){
            ...
            $coindata=Coin::whereGate_id($request->block_gateway)->first();
            $gate=$request->block_gateway;
            $send_coin=floor(($request->pay_amount+$charge/$coindata->price)*10000000)/100000000;
            //get the balance
            $key = $request->account_info;
            $baseUrl = "https://block.io";
            $endpoint = "/api/v2/get_balance/?api_key=".$key."";
            ...
            if($bal<$send_coin){
                return back()->with("alert", "You dont have enough ".$coindata->name ." in your ".$coindata->name ." wallet.Please deposit more ".$coindata->name ." or try using another payment gateway");
            }
            //end
            ...
            //send coin
            $baseUrl = "https://block.io";
            $endpoint = "/api/v2/get_network_fee_estimate/?api_key=".$key."&amounts=".$send_coin."&to_addresses=".$coindata->address."";
            $httpVerb = "GET";
            $contentType = "application/json"; //e.g charset=utf-8
            $headers = array (
                "Content-Type: $contentType",
                ...
                if($content['status'] == "success") {
                 ...
                 $get=$request->amount;
                 $buy['currency_id'] = $currency->id;
                 $buy['main_amo'] = $usd*$coindata->price+$charge;
                 $buy['charge'] = $charge;
                 $buy['price'] = $currency->price;
                 $buy['getamo'] = $get;
                 $buy['user_id'] = Auth::id();
            ...
            // start stellar transaction
            //Account Exist
            // try {
            //     $exists = $server->accountExists($received_public_key);
            
            //     if ($exists) {
            //     }
            //     else {
            //         return redirect()->back()->with("alert", "Account does not exist!");
            //     }
            // }
            // catch (\Exception $e) {
        ...
        return redirect()->back()->with('message', 'Successfully Deposit Smooth Coin');
    }
...
public function save_wallet(Request $request) {
    ...
    if($request->payment==1){
        ...
        User::find($user_id)->update(array(
            'stellarwallet' => $request->public_key,
        ));
    }
    else{
        $this->validate($request, [
    ...
    $admin_private_key=config('paypal.admin_private_key');
    $admin_public_key=config('paypal.admin_public_key');
    $receivingKeypair = $private_key;
    $server = Server::publicNet();
    $asset = Asset::newCustomAsset('SMC', $issuingKeypair);
    
    try {
        $response = $server->buildTransaction($admin_public_key)
        ->addLumenPayment($public_key, 2) // this will default to the maximum value
        ->getTransactionEnvelope();
        $response->sign($admin_private_key);
        $b64Tx = base64_encode($response->toXdr());
        $server->submitB64Transaction($b64Tx);
    ...
    // Second, the receiving account must add a trustline for the issuer
    try {
        $response = $server->buildTransaction($public_key)
    ...
    return redirect()->back()->with('message', 'Succussfully added');
}
///////////////////////////End Add Wallet////////////////////////////

// public function trustline() {
//     return view('user.trustline');
// }

// public function save_trustline(Request $request) {
//     $private_key = $request->private_key;
//     $user = User::find(Auth::user()->id);
//     $public_key=$user->stellarwallet;
//     if($public_key==""){
//         return redirect('user/payment-account')->with("alert", "You need register stellar wallet, Please register wallet address");
//     }
//     $server = Server::publicNet();
...
//     $asset = Asset::newCustomAsset('SMH', $issuingKeypair);
...
public function sellecoin(Request $request)
{
    ...
    if($request->flag==4){
        ...
        $client_private_key=$request->paypal_private_key;
    }
    elseif($request->flag==3){
        $this->validate($request, [
            'wallet' => 'required',
            ...
            'payment' => 'required',
            'pay_amount'=>'required',
            'stellarwallet_private_key'=>'required',
            // 'paywallet'=>'required',
            // 'paypal_address'=>'required',
        ]);
        $client_private_key=$request->stellarwallet_private_key;
    }
    else{
        $this->validate($request, [
            'wallet' => 'required',
    ...
    If(flag==0){
        return back()->with("alert", "You dont have enough ".$currency->symbol ." in your stellar wallet.Please deposit more ".$currency->symbol." or try using another payment gateway"); 
    }
    //end balance
    if($request->flag==4){
        //payment
        $send_coin = $request->amount;
        $asset = Asset::newCustomAsset($currency->symbol, $issuingKeypair);
            try {
                $response = $server->buildTransaction($client_public_key)
                ->addCustomAssetPaymentOp($asset, $send_coin, $admin_public_key) // this will default to the maximum value
                ->getTransactionEnvelope();
        ...
        $buy['currency_id'] = $currency->id;
        $buy['amount']=$send_coin*$currency->sell;
        $buy['main_amo'] = $send_coin*$currency->sell-$charge;
        $buy['charge'] = $charge;
        $buy['price'] = $currency->price;
        $buy['getamo'] = $reuest->pay_amount-$charge;
        $buy['user_id'] = Auth::id();
        $buy['type'] = 0;
        $buy['wallet'] = $request->paypal_address;
        $buy['gateway']='101';
    ...
    elseif($request->flag==2){
        ...
        $get=$request->pay_amount-$charge/$coindata->price;
    ...
    elseif($request->flag==3){
        ...
        $baseUrl = "https://block.io";
        $endpoint = "/api/v2/get_network_fee_estimate/?api_key=".$key."&amounts=".$get."&to_addresses=".$address."";
        $httpVerb = "GET";
        $contentType = "application/json"; //e.g charset=utf-8
        $headers = array (
            "Content-Type: $contentType",
    ...
    return redirect()->back()->with('message', 'Successfully Withdraw Smooth Coin');
}
